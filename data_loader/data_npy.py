...
class DataGenerator(Dataset):
    ...
    def _scan(self):
        names_hr = sorted(glob.glob(path.join(self.hr_dir, "*.npy")))
        names_lr = sorted(glob.glob(path.join(self.lr_dir, "*.npy")))
    ...
    def _load_file(self, index):
        lr_data = self._npy_loader(self.lr_list[index])
        hr_data = self._npy_loader(self.hr_list[index])
...
class DIV2K_train(DataGenerator):
    ...
    def _scan(self):
        names_hr, names_lr = super(DIV2K_train, self)._scan()
        names_hr = names_hr[:800]
        names_lr = names_lr[:800]
        return names_hr, names_lr
...
class DIV2K_val(DataGenerator):
    ...
    def _scan(self):
        names_hr, names_lr = super(DIV2K_val, self)._scan()
        names_hr = names_hr[800:810]
        names_lr = names_lr[800:810]
        return names_hr, names_lr

class DF2K_train(DataGenerator):
    def __init__(self, opt, data_dir, data_name='DF2K', train=True):
        super(DF2K_train, self).__init__(opt, data_dir=data_dir, data_name=data_name, train=train)
    ...
    def _set_filesystem(self, data_dir):
        self.apath = path.join(data_dir, self.name)
        self.hr_dir = path.join(self.apath, 'DF2K_train_HR')
        self.lr_dir = path.join(self.apath, 'DF2K_train_LR_bicubic/X{}'.format(self.upscaling_factor))
    
    def _scan(self):
        names_hr, names_lr = super(DF2K_train, self)._scan()
        names_hr = names_hr[:3450]
        names_lr = names_lr[:3450]
        return names_hr, names_lr

class DF2K_val(DataGenerator):
    def __init__(self, opt, data_dir, data_name='DF2K', train=False):
        super(DF2K_val, self).__init__(opt, data_dir=data_dir, data_name=data_name, train=train)
    ...
    def _scan(self):
        names_hr, names_lr = super(DF2K_val, self)._scan()
        names_hr = names_hr[3450:3460]
        names_lr = names_lr[3450:3460]
        return names_hr, names_lr
...
if __name__ == "__main__":
    ...
    #     print('filename:{} || LR_Shape:{} || HR:{}'.format(train_data[i][2], train_data[i][0].shape, train_data[i][1].shape))

    train_data = DF2K_train(data_dir=data_dir, data_name=dataset)
    print(len(train_data))
    val_data = DF2K_val(data_dir=data_dir, data_name=dataset)
    print(len(val_data))
